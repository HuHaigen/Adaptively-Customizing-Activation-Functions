...
training_epochs =150 #80 #150

parser.add_argument("--model_type", dest='model_type', default='GoogLeNet', help='type of model')
...
def loss_to_excel(step, cross_entropy, AdaptiveFun_name):
    path = os.path.join('./result_googlenet/cifar10_googlenet_loss_2.xlsx')
...
def evolution_to_excel(epoch, acc_test, pre_test, rec_test, f1_test, AdaptiveFun_name):
    path = os.path.join('./result_googlenet/cifar10_googlenet_evolution_2.xls')
    p = os.path.exists(path)
...
def main():
    ...
    loss_to_excel(training_epochs, cross_entropy_, 'Prelu')
    evolution_to_excel(training_epochs, acc_test, pre_test, rec_test, f1_test, 'Prelu')
    plt.plot(cross_entropy_, '-k', label='Prelu')
